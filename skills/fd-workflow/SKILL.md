---
name: fd-workflow
description: Manage a lightweight Feature Design (FD) workflow in software repositories. Use when the user asks to initialize FD tracking, create a feature design, inspect project/FD status, perform deep multi-angle design analysis, verify an implementation, or close/archive an FD. Designed to work across OpenAI Codex CLI and GitHub Copilot CLI without depending on Claude-specific commands, tool names, or model names.
---
...
4. Run the smallest meaningful build/test/lint/static checks first; broaden only when failures or risk justify it.
