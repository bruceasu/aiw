...
1. **Logical profile** ── what this skill emits:
   - `tasks.md` (always, when running standalone)
...
openspec/
  changes/
    <change-id>/
      ...
      tasks.md            # produced here by office-hours-finance
...
|---|---|---|
| `## Recommendation` | `tasks.md` head + `task.toml` `[intake].recommendation` | Enum: `PROCEED` / `HOLD` / `REDUCE` / `NEEDS_VALIDATION`. |
...
| `## 1. Problem` | `tasks.md` | 1:1. Keep sub-sections 1.1 / 1.2 / 1.3 / 1.4. |
| `## 2. Decision Flow` | `tasks.md` | 1:1. Every column must be filled. |
| `## 3. Stakeholders` | `tasks.md` | 1:1. Every row that says "Operational Owner? yes" becomes a required approver on `task.toml`. |
| `## 4. Scope` | `tasks.md` | 1:1. Sub-sections 4.1 / 4.2 / 4.3 / 4.4. Non-Goals must be preserved verbatim through downstream skills. |
| `## 5. Unknowns` | `tasks.md` + `tasks.md` | Every row with `Blocks Next Step? yes` becomes a task in `tasks.md`; others become follow-ups. |
...
`tasks.md` (or `Problem Brief` draft) file against the Output Format
headings. It does not walk the whole `openspec/` tree.

## Backward Compatibility

- Never delete a heading from `tasks.md` even when the section is
