#!/usr/bin/env python3
"""Validate a METRICS_SPEC.md (or metrics.md) against the metrics-review
Output Format.
...
Usage:
  python scripts/validate_metrics_review.py <path-to-md-file>
  python scripts/validate_metrics_review.py <path-to-md-file> --strict
  python scripts/validate_metrics_review.py <path-to-md-file> --json
...
def main(argv: Optional[Iterable[str]] = None) -> int:
    parser = argparse.ArgumentParser(
        description="Validate a METRICS_SPEC.md against metrics-review."
    )
    ...
    if args.json:
        ...
        if ok:
            print("Metrics Spec validation passed.")
        else:
            print("Metrics Spec validation failed:")
            for e in errors:
                print(f"- {e}")
