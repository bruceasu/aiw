...
func Dispatch(args []string) error {
	...
	uiFactory := func() UI {
		return NewPromptUI(DraftFromLLMWithSelector, PromptLine, PromptMultilineWithEditor)
	}
	defaultUI := NewDefaultUI(IsInteractiveTerminal(), PromptLine, PromptMultilineWithEditor, uiFactory)
	return dispatchWithUI(args, defaultUI, stagedChanges, commitWithMessage)
}

func dispatchWithUI(args []string, u UI, stagedFn func() (string, error), commitFn func(string) error) error {
...
func loadCzConfig(opts czOptions) (Config, error) {
	...
	if global, err := ai.LoadConfig(); err != nil {
		...
		if global.Command != "" {
			...
			if global.Name == "copilot" || global.Name == "copilot-cli" {
				cfg.CopilotCommand = global.Command
			}
		}
	}
	return cfg, nil
...
// commands without duplicating aiw.toml/.aiw.toml parsing or precedence rules.
func LoadAIProviderConfig() (ai.Config, error) {
...
func finalizeConfig(cfg *Config) {
	...
	case "codex", "codex-cli", "copilot", "copilot-cli", "":
		// Keep generic values unchanged; provider selection/fallback is handled in RunLLM.
	}
}
...
func mergeCzConfigFromTomlFile(cfg *Config, path string) error {
...
func DraftFromLLMWithSelector(cfg Config, selector func(Config, []Draft) (Draft, error)) (Draft, error) {
	for {
...
`, cfg.Candidates, strings.Join(typeList, ", "), files, hist, len(diff), diff)
		...
		out, err := ai.RunLLM(prompt, ai.LLMConfig{
			Provider: cfg.LLMProvider, Model: cfg.LLMModel, APIBaseURL: cfg.APIBaseURL,
			APIKey: cfg.APIKey, OpenAIKey: cfg.OpenAIKey, GeminiKey: cfg.GeminiKey,
			CodexCommand: cfg.CodexCommand, CopilotCommand: cfg.CopilotCommand,
			DryRun: DryRun,
		}, czCandidatesSchema())
		if err != nil {
			return Draft{}, err
		...
		if err != nil {
			return Draft{}, fmt.Errorf("parse llm output: %w", err)
		}
		if len(cands) == 0 {
			return Draft{}, errors.New("llm returned no candidates")
		}
		if len(cands) == 1 {
		...
		d = SanitizeDraft(d, cfg)
		d.Footer = filterLLMIssueFooter(d.Footer, allowedIssueRefs)
		return d, nil
	}
}

func selectDraftCandidateFromPrompt(cfg Config, cands []Draft) (Draft, error) {
	fmt.Println("\nLLM candidates:")
	for i, c := range cands {
		preview := BuildHeader(c)
		fmt.Printf("  %d) %s\n", i+1, preview)
	}
	for {
		...
		case "q", "quit", "n", "no":
			return Draft{}, errors.New("aborted")
		case "r", "regen":
...
func draftFromLastCommit() (Draft, error) {
	...
	if len(lines) == 0 {
		return Draft{}, errors.New("empty last commit message")
	}
	header := strings.TrimSpace(lines[0])
	d := Draft{}
	if parsed, ok := ParseConventionalHeader(header); ok {
		d.Type = parsed.Type
