...
func Dispatch(args []string) error {
	return dispatchWithUI(args, newDefaultUI(isInteractiveTerminal()), stagedChanges, commitWithMessage)
}
...
func mergeCzConfigFromTomlFile(cfg *czConfig, path string) error {
	...
	applyScope := func() {
		...
		if v != "" {
			...
			cfg.Scopes = append(cfg.Scopes, czScope{Value: v, Name: n})
		}
		curScope = map[string]string{}
		hasScope = false
	}
...
func czDraftFromLLM(cfg czConfig) (czDraft, error) {
	return czDraftFromLLMWithSelector(cfg, selectDraftCandidateFromPrompt)
}

func czDraftFromLLMWithSelector(cfg czConfig, selector func(czConfig, []czDraft) (czDraft, error)) (czDraft, error) {
	for {
...
func selectDraftCandidateFromPrompt(cfg czConfig, cands []czDraft) (czDraft, error) {
	fmt.Println("\nLLM candidates:")
	for i, c := range cands {
...
func draftFromLastCommit() (czDraft, error) {
	msg, err := gitOutput("git", "log", "-1", "--pretty=%B")
	if err != nil {
		return czDraft{}, err
	}
	lines := strings.Split(strings.TrimRight(msg, "\n"), "\n")
	if len(lines) == 0 {
		return czDraft{}, errors.New("empty last commit message")
	}
...
func promptFooterPrefix(cfg czConfig) string {
	...
	for {
		in := strings.TrimSpace(promptLine("Choose prefix # or value: "))
		if in == "" {
			return ""
		}
		if idx, err := strconv.Atoi(in); err == nil {
			if idx >= 1 && idx <= len(options) {
				sel := options[idx-1]
				if sel == "custom" {
					return strings.TrimSpace(promptLine(cfg.Messages.CustomFooterPrefix + " "))
				}
				if sel == "skip" {
					return ""
				}
...
func buildCommitMessage(d czDraft) string {
	d = czDraft{
...
func buildHeader(d czDraft) string {
	header := d.Type
	if d.Scope != "" {
		header += "(" + d.Scope + ")"
	}
