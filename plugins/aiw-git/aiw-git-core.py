...
implementation to be used by per-subcommand wrappers such as git-show.py.
"""
import sys
import subprocess
import os
...
implement dispatch for subcommands; each `git-<sub>.py` wrapper
...
def generate_md_from_meta(meta, outpath):
    ...
    if meta.get('args'):
        ...
        for a in meta['args']:
            lines.append(f"- {a.get('flag','')} 鈥?{a.get('description','')}")
        lines.append("")
    if meta.get('examples'):
...
if __name__ == '__main__':
    print('aiw-git-core is a library; run one of the git-<sub>.py wrappers')
    sys.exit(2)
    print(info['short'])
    print()
    print(info['long'])
