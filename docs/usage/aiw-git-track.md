...
Arguments:
- <branch> 鈥?The branch to track or set upstream for.
- [remote] 鈥?The remote to track the branch from (default: origin).
- --push 鈥?Push the branch to the remote and set upstream.
- --track 鈥?Create a new local tracking branch from the remote branch.
