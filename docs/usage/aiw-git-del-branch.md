...
Arguments:
- <branch> 鈥?The name of the branch to delete.
- --force 鈥?Force-delete local branch even if not merged (-D).
- --remote 鈥?Delete local branch AND remote branch.
- --remote-only 鈥?Delete only remote branch.
- --remote-name 鈥?Remote name to use (default: origin).
