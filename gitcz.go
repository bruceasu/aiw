...
func gitCz(args []string) error {
	...
	var draft czDraft
	if cfg.UseLLM {
		draft, err = czDraftFromLLM(cfg)
		if err != nil {
			return err
		}
	} else {
		draft, err = czDraftFromWizard(cfg)
		if err != nil {
			return err
		}
	}
...
func loadOpenAIEnvFromDotEnv() (map[string]string, map[string]string, error) {
	exeLoader := &Loader{Env: map[string]string{}}
	cwdLoader := &Loader{Env: map[string]string{}}
